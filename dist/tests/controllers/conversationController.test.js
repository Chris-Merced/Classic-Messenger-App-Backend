"use strict";
const { checkDirectMessageConversation, addMessageToConversations, getOnlineUsers, } = require('../../controllers/conversationController');
...
describe('conversationController Unit Testing for Crucial Functions', () => {
    test('returns 200 + id when profile is public', async () => {
        ...
        expect(db.checkDirectMessageConversationExists).toHaveBeenCalledWith('1', '2');
    ...
    test('stores message when not blocked', async () => {
        ...
        const res = makeRes();
        await addMessageToConversations(req, res);
        expect(db.addMessageToConversations).toHaveBeenCalledWith(JSON.stringify(body));
        expect(res.status).toHaveBeenCalledWith(200);
    });
    test('maps userList → online status from Redis', async () => {
        ...
        expect(redisPublisher.hGet).toHaveBeenNthCalledWith(1, 'activeUsers', 'alice');
...
//# sourceMappingURL=conversationController.test.js.map