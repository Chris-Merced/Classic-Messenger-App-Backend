...
async function addUser(user) {
    try {
        ...
        let emailData = await pool.query("SELECT * FROM users WHERE LOWER(email)=LOWER($1)", [user.email.trim()]);
        if (emailData.rows[0]) {
            console.log("Email already in use");
            throw new Error("Email Already In Use");
        }
        await pool.query("INSERT INTO users (username,password, email) VALUES ($1, $2, $3)", [user.username.trim(), user.password, user.email.trim().toLowerCase()]);
...
async function addUserOAuth(email, username) {
    try {
        ...
        const response = await pool.query("INSERT INTO users (username,password, email) VALUES ($1, $2, $3)", [username.trim(), hashedPassword, email.trim().toLowerCase()]);
...
async function getUserIDByUsername(username) {
    try {
        const { rows } = await pool.query("SELECT id FROM users WHERE LOWER(username)=LOWER($1)", [username.toLowerCase()]);
        if (rows[0]) {
            return rows[0].id;
        }
        else {
...
async function getUsersByUsernameSearch(username, page, limit) {
    try {
        ...
        const { rows } = await pool.query("SELECT * FROM USERS WHERE username ILIKE $1 LIMIT $2 OFFSET $3", [`%${username}%`, limit, offset]);
...
async function checkEmailExists(email) {
    try {
        const { rows } = await pool.query("SELECT * FROM users WHERE LOWER(email)=LOWER($1)", [email.trim().toLowerCase()]);
        if (rows[0]) {
            return rows[0];
        }
        else {
...
async function checkUsernameExists(username) {
    try {
        const { rows } = await pool.query("SELECT * FROM users WHERE username ILIKE $1", [username.trim()]);
        if (rows[0]) {
            return true;
        }
        else {
...
async function checkSession(token, userID) {
    try {
        ...
        const { rows } = await pool.query("SELECT * FROM sessions WHERE session_id = $1 AND user_id = $2", [JSON.parse(token).sessionID, userID]);
        if (rows[0]) {
            return true;
        }
        else {
...
async function checkIfParticipant(data) {
    try {
        ...
        console.err("Error Checking if User is a participant of conversation: \n" + err);
...
async function addParticipant(conversation_id, user_id) {
    try {
        await pool.query("INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2)", [conversation_id, user_id]);
    }
    catch (err) {
...
async function getParticipantsByConversationID(conversationID) {
    try {
        ...
        console.error("Error finding conversation participants by conversation ID: " +
            err.message);
        throw new Error("Error finding conversation participants by conversation ID " +
            err.message);
    }
}
async function addMessage(data) {
    try {
        const response = await pool.query("INSERT INTO messages (conversation_id, sender_id, content) VALUES ($1, $2, $3)", [data.conversationID, data.userID, data.message]);
        return response;
    }
    catch (err) {
...
async function getUserIDByConversationID(conversationID, userID) {
    try {
        const { rows } = await pool.query("SELECT user_id FROM conversation_participants WHERE conversation_id=$1 AND user_id!=$2", [conversationID, userID]);
        if (rows[0]) {
...
async function addFriendRequestToDatabase(userID, profileID) {
    try {
        await pool.query("INSERT INTO friend_requests (user_id, request_id, status) VALUES ($1, $2, $3)", [userID, profileID, "pending"]);
    }
    catch (err) {
        return err.message;
    }
...
async function getFriendRequests(userID) {
    try {
        const { rows } = await pool.query("SELECT * FROM friend_requests WHERE request_id=$1", [userID]);
...
async function addFriend(userID, requestID) {
    ...
    try {
        ...
        await pool.query("INSERT INTO friends (user_id, friend_id) VALUES ($1, $2)", [smaller, larger]);
        await pool.query(`
      DELETE FROM 
        friend_requests 
      WHERE 
...
async function denyFriend(userID, requestID) {
    try {
        const response = await pool.query("DELETE FROM friend_requests WHERE user_ID=$1 AND request_id=$2", [userID, requestID]);
    }
    catch (err) {
...
async function checkIfBlocked(userID, blockedID) {
    try {
        const { rows } = await pool.query("SELECT * FROM blocked WHERE user_id=$1 AND blocked_id=$2", [userID, blockedID]);
        if (rows[0]) {
            return true;
        }
        else {
...
async function editAboutMe(aboutMe, userID) {
    try {
        const { rows } = await pool.query("UPDATE users SET about_me=$1 WHERE id=$2 RETURNING *", [aboutMe, userID]);
        return rows[0];
    }
    catch (err) {
...
async function checkAdminStatus(token, id) {
    try {
        const { rows } = await pool.query(`SELECT users.is_admin, sessions.session_id 
      FROM users 
      JOIN sessions ON users.id=sessions.user_id 
      WHERE users.id=$1 AND sessions.session_id=$2`, [id, token]);
...
async function banUser(banExpiresAt, id) {
    try {
        ...
        const bannedResult = await pool.query("UPDATE users SET banned=TRUE WHERE id=$1 RETURNING id", [id]);
        if (bannedResult.rowCount === 0) {
            return false;
        }
        if (banExpiresAt !== "perm") {
            const { rowCount } = await pool.query("UPDATE users SET ban_expires=$1 WHERE id=$2", [banExpiresAt, id]);
            timeUpdateResult = rowCount > 0;
        }
        if (banExpiresAt === "perm") {
            const { rowCount } = await pool.query("UPDATE users SET ban_expires=NULL WHERE id=$1", [id]);
            timeUpdateResult = rowCount > 0;
        }
        if (timeUpdateResult) {
            return true;
...
async function unbanUser(id) {
    try {
        const { rowCount } = await pool.query("UPDATE users SET banned=false, ban_expires=null WHERE id=$1", [id]);
        if (rowCount > 0) {
            return true;
        }
        else {
...
async function makeAdmin(id) {
    try {
        const { rowCount } = await pool.query("UPDATE users SET is_admin=true WHERE id=$1", [id]);
        if (rowCount > 0) {
            return true;
        }
        else {
...
//# sourceMappingURL=queriesOld.js.map